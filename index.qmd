...
citation:
  url: https://washingtonsilva.github.io/dbca/
lang: "pt"
date: today
...
Tornou-se um padrão reportar tamanhos de efeitos nos artigos científicos de 
diversas áreas, por diversas razões, para entende-las veja @ferguson2016, 
@hummel2019.

Para ilustrar o conceito de tamanho do efeito para uma Análise da Variância, 
no contexto do experimento analisado, podemos perguntar qual é a porcentagem 
da variação total no crescimento de bactérias associada aos tratamentos 
(soluções de lavagem). Esta medida é chamada de *Eta-quadrático* ($\eta^{2}$):
...
Quando adicionamos mais termos ao modelo que estamos analisando, como é o caso do experimento com a inclusão de blocos, podemos fazer duas perguntas diferentes 
sobre a porcentagem de variância explicada por um preditor - quanta variância é explicada pelo fator no total (38% no caso dos tratamentos) e quanto da 
variação é explicada *ao controlar quaisquer outros fatores*. 

A última pergunta é respondida pelo Eta-parcial quadrático $\eta^{2}_{p}$, que 
é a porcentagem da variância *parcial* (após controlar outros fatores no 
